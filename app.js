...
function renderRow(p, blur) {
  ...
  tr.innerHTML = `<td><strong>${p.name}</strong></td>` + cell(p.us, "us") + cell(p.uk, "uk");
...
function renderComplexity(p, blur) {
  ...
  html += `</div><p class="fineprint">US: national average = DRG weight × $7,276.76 (FY2026 operating + capital standardized amounts), before geographic adjustment. UK: 2025/26 elective unit price, before Market Forces Factor. Sources: ${SOURCES.us}; ${SOURCES.uk}.</p>`;
